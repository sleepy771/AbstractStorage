...
public class Controller {
	...
	public Object load(final GlobalPath p) throws NameNotFoundException,
		...
		Callable<Object> call = new Callable<Object>() {
			...
			public Object call() throws Exception {
				...
				Class<?> clazz = Class.forName(dat.getClassName());
	...
	public <T> T loadType(final GlobalPath p) throws InterruptedException,
			ExecutionException {
	...
	public Map<String, Object> loadAll(final Collection<GlobalPath> paths)
			throws NotBuildableException, NameNotFoundException,
			InterruptedException, ExecutionException {
	...
	public <T> Map<String, T> loadAllTypeDefined(
			final Collection<GlobalPath> paths) throws NotBuildableException,
			InterruptedException, ExecutionException {
	...
	public Map<String, Future<Object>> loadAllFuture(
			Collection<GlobalPath> paths) {
	...
	public void loadSilent(GlobalPath p, final ObjectReference<Object> ref) {
		...
		collector.addFutureWithOperation(p.getObjectName(), loadFuture(p),
				operation);
	}

	public void loadAllSilent(Collection<GlobalPath> paths,
		...
		collector.addAllFutureObjectsWithOperation(loadAllFuture(paths),
				operation);
	}
	...
	public void saveAndWait(final GlobalPath p, final Object o)
	...
	public void saveSilent(final GlobalPath p, final Object o) {
		Runnable silentSaveRunnable = new Runnable() {
			...
			public void run() {
				...
				try {
					serializer = serializers.instantiateSerializerFor(o
							.getClass());
					...
					storage.store(p.getLocal(), data);
						...
						| UnserializableObjectException | NameNotFoundException e) {
	...
	public Data serialize(Object o, String name) throws InterruptedException,
			ExecutionException {
		return serializeFuture(o, name).get();
	}

	public void save(GlobalPath p, Data d) throws InterruptedException,
			ExecutionException {
		saveFuture(p, d).get();
	}
	...
	private Future<Void> saveFutureWithNotify(final GlobalPath p, final Data d,
			final StorageListener<Void> obs) {
		Callable<Void> saveTask = new Callable<Void>() {
	...
	private Future<Data> serializeFuture(final Object o, final String name) {
		Callable<Data> serialTask = new Callable<Data>() {
			...
			public Data call() throws Exception {
				Serializer s = serializers.instantiateSerializerFor(o
						.getClass());
	...
	private Future<Data> serializeFutureWithNotify(final Object o,
			final String name, final StorageListener<Void> obs) {
